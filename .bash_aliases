...
alias gfd='git status --porcelain | grep -E "^(A|\s|M)M\s" | sed "s/\(A\|\sM\|MM\)\s//g" |  head -1 | xargs git diff'
alias gfa='git status --porcelain | grep -E "^(A|\s|M)M\s" | sed "s/\(A\|\sM\|MM\)\s//g" |  head -1 | xargs git add'
alias gfr='git status --porcelain | grep -E "^(A|\s|M)M\s" | sed "s/\(A\|\sM\|MM\)\s//g" |  head -1 | xargs git checkout HEAD'
alias ga='git add'
