...
alias vpn-connect='printf "y\ny.fedoruk\nA2yfedoruck\ny" | /opt/cisco/anyconnect/bin/vpn -s connect 195.66.110.158'
...
alias gfd='git status --porcelain | grep -E "^(\s|A|\s|M)M\s" | sed "s/\(A\|\s\|M\|M\)M\s//g" |  head -1 | xargs git diff'
alias gfa='git status --porcelain | grep -E "^(A|\s|M)M\s" | sed "s/\(A\|\s\|M\|M\)M\s//g" |  head -1 | xargs git add'
alias gfr='git status --porcelain | grep -E "^(A|\s|M)M\s" | sed "s/\(A\|\s\|M\|M\)M\s//g" |  head -1 | xargs git checkout HEAD'
alias ga='git add'
