...
alias gfd='git status --porcelain | grep "^ M " | sed "s/ M //g" |  head -1 | xargs git diff'
alias gfa='git status --porcelain | grep "^ M " | sed "s/ M //g" |  head -1 | xargs git add'
alias ga='git add'
...
alias loc='rm -rf ~/www/loc.inmyvac.com/protected/config/environments/local/ && cp -r ~/www/loc.inmyvac.com/zsys/local ~/www/loc.inmyvac.com/protected/config/environments'
